...
public class ReflectiveHierarchyStep {
    ...
    public ReflectiveHierarchyIgnoreWarningBuildItem ignoreJavaClassWarnings() {
        return new ReflectiveHierarchyIgnoreWarningBuildItem(
                ReflectiveHierarchyBuildItem.IgnoreWhiteListedPredicate.INSTANCE);
    }

    @BuildStep
    public void build(CombinedIndexBuildItem combinedIndexBuildItem, ExtraIndexBuildItem extraIndexBuildItem,
        ...
        if (!finalFieldsWritablePredicates.isEmpty()) {
            // create a predicate that returns true if any of the predicates says that final
            // fields need to be writable
            finalFieldsWritable = finalFieldsWritablePredicates.stream()
        ...
        for (ReflectiveHierarchyBuildItem i : hierarchy) {
            addReflectiveHierarchy(combinedIndexBuildItem, extraIndexBuildItem, i,
                    i.hasSource() ? i.getSource() : i.getType().name().toString(), i.getType(),
                    processedReflectiveHierarchies, unindexedClasses, finalFieldsWritable, reflectiveClass);
        }
    ...
    private void removeIgnored(Map<DotName, Set<String>> unindexedClasses,
        ...
        // the final predicate ignores a DotName if and only if at least one of the
        // predicates indicates that warning should be ignored
    ...
    private void addReflectiveHierarchy(CombinedIndexBuildItem combinedIndexBuildItem,
            ExtraIndexBuildItem extraIndexBuildItem, ReflectiveHierarchyBuildItem reflectiveHierarchyBuildItem,
            String source, Type type, Set<DotName> processedReflectiveHierarchies,
            Map<DotName, Set<String>> unindexedClasses, Predicate<ClassInfo> finalFieldsWritable,
        ...
        if (type instanceof VoidType || type instanceof PrimitiveType || type instanceof UnresolvedTypeVariable) {
            ...
            CompositeIndex index = CompositeIndex.create(combinedIndexBuildItem.getIndex(),
                    extraIndexBuildItem.getIndex());

            addClassTypeHierarchy(combinedIndexBuildItem, extraIndexBuildItem, reflectiveHierarchyBuildItem, source,
                    type.name(), processedReflectiveHierarchies, unindexedClasses, finalFieldsWritable,
                    reflectiveClass);

            for (ClassInfo subclass : index.getAllKnownSubclasses(type.name())) {
                addClassTypeHierarchy(combinedIndexBuildItem, extraIndexBuildItem, reflectiveHierarchyBuildItem, source,
                        subclass.name(), processedReflectiveHierarchies, unindexedClasses, finalFieldsWritable,
                        reflectiveClass);
            }
            ...
            addReflectiveHierarchy(combinedIndexBuildItem, extraIndexBuildItem, reflectiveHierarchyBuildItem, source,
                    type.asArrayType().component(), processedReflectiveHierarchies, unindexedClasses,
            ...
            if (!reflectiveHierarchyBuildItem.getIgnorePredicate().test(parameterizedType.name())) {
                addClassTypeHierarchy(combinedIndexBuildItem, extraIndexBuildItem, reflectiveHierarchyBuildItem, source,
                        parameterizedType.name(), processedReflectiveHierarchies, unindexedClasses, finalFieldsWritable,
                        reflectiveClass);
            }
            for (Type typeArgument : parameterizedType.arguments()) {
                addReflectiveHierarchy(combinedIndexBuildItem, extraIndexBuildItem, reflectiveHierarchyBuildItem,
                        source, typeArgument, processedReflectiveHierarchies, unindexedClasses, finalFieldsWritable,
                        reflectiveClass);
            }
        }
    }
    ...
    private void addClassTypeHierarchy(CombinedIndexBuildItem combinedIndexBuildItem,
            ExtraIndexBuildItem extraIndexBuildItem, ReflectiveHierarchyBuildItem reflectiveHierarchyBuildItem,
            String source, DotName name, Set<DotName> processedReflectiveHierarchies,
            Map<DotName, Set<String>> unindexedClasses, Predicate<ClassInfo> finalFieldsWritable,
            BuildProducer<ReflectiveClassBuildItem> reflectiveClass) {
        if (reflectiveHierarchyBuildItem.getIgnorePredicate().test(name)) {
            return;
        }
        CompositeIndex index = CompositeIndex.create(combinedIndexBuildItem.getIndex(), extraIndexBuildItem.getIndex());
        ...
        reflectiveClass.produce(ReflectiveClassBuildItem.builder(name.toString()).methods(true).fields(true)
                .finalFieldsWritable(doFinalFieldsNeedToBeWritable(info, finalFieldsWritable)).build());
        ...
        addClassTypeHierarchy(combinedIndexBuildItem, extraIndexBuildItem, reflectiveHierarchyBuildItem, source,
                info.superName(), processedReflectiveHierarchies, unindexedClasses, finalFieldsWritable,
                reflectiveClass);
        for (FieldInfo field : info.fields()) {
            if (Modifier.isStatic(field.flags()) || field.name().startsWith("this$")
                    || field.name().startsWith("val$")) {
                // skip the static fields (especially loggers)
                // also skip the outer class elements (unfortunately, we don't have a way to
                // test for synthetic fields in Jandex)
                continue;
            }
            addReflectiveHierarchy(combinedIndexBuildItem, extraIndexBuildItem, reflectiveHierarchyBuildItem, source,
                    field.type(), processedReflectiveHierarchies, unindexedClasses, finalFieldsWritable,
                    reflectiveClass);
        }
        for (MethodInfo method : info.methods()) {
            ...
            addReflectiveHierarchy(combinedIndexBuildItem, extraIndexBuildItem, reflectiveHierarchyBuildItem, source,
                    method.returnType(), processedReflectiveHierarchies, unindexedClasses, finalFieldsWritable,
                    reflectiveClass);
        }
    }
